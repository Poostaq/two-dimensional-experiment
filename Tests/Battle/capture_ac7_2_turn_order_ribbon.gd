...
func _run() -> void:
	...
	await _click(arena.get_node("%DefaultAttackButton"))
	...
	_expect(not (arena.get_node("%DefaultActionConfirmButton") as Button).disabled, "ribbon preserves selected attack target")
	...
	await _click(arena.get_node("%DefaultActionCancelButton"))
