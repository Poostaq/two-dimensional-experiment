...
Implementation status reviewed on 2026-09-20: AC8.1-AC8.6 are implemented and verified for the current legacy-v1 world, including AC8.4 habitat/debug presentation and full filtered offers. The separately requested standalone dismissal extension also passes its D1 minimum-one-member/no-refund policy. AC8.7-AC8.8 and AC9-AC10 remain incomplete; their unchecked criteria below define planned behavior. They replace reward-choice recruitment and move-count-based Sudden Death. Cross-run progression and unrelated AC5 questions remain deferred.
...
**Interim rule before habitats:** Every valid hex in world version 1 is classified as Goblin habitat through the explicit legacy compatibility rule; town ownership retains town-only validation. Current-world Goblin town recruitment and whole-roster eligibility are verified under AC8.5/AC8.6. Implement AC8 against the existing towns without waiting for habitat generation. This does not change their count, placement or roads. Once habitats are implemented, habitat-enabled worlds use each town's explicit owning clan; preserve the Goblin-town rule for pre-habitat world versions rather than silently reassigning existing saves.

Purchases revalidate class absence, town availability and funds at commit. Roster placement/replacement and the 500g deduction form one durable transaction. Cancellation, failed saves and repeated/stale requests must not charge or add a duplicate. Gold awards are settled once per battle and survive reload without duplication. A victorious ordinary combat hex becomes a safe encounter on revisit and after Continue; stale requests cannot reopen its battle or award gold again. Closing an unfinished encounter does not count as a victory. This regression is verified in [cleared-hex revisit evidence](AC8/Evidence/battle-revisit-fix/verification.md). Existing post-battle recovery remains in effect.
...
- [x] AC7.6 — Active battle skills have equal rendered button dimensions and hover/focus descriptions while default controls retain their sizes. Right-clicking an allied or enemy character opens a left-side information panel with name/class-role, HP, speed, base damage, flat damage reduction, buffs/debuffs with accurate expiry, and visible passive descriptions. Inspection uses complete committed snapshots, preserves pending battle actions, and obeys explicit modal, pointer, keyboard and focus precedence. Passives leave the battle action bar without changing their mechanics. ([plan](../superpowers/plans/2026-09-18-battle-skill-and-character-information.md), [verification — PASS](AC7/Evidence/AC7.6/verification.md))
...
Plan: [AC8 implementation plan](../superpowers/plans/2026-09-18-ac8-town-recruitment-and-gold.md). AC8.1-AC8.6 pass for the current world, including AC8.4 filtered offers. AC8.7-AC8.8 remain unchecked. See [AC8.2 evidence](AC8/Evidence/AC8.2/verification.md), [AC8.3 evidence](AC8/Evidence/AC8.3/verification.md), [AC8.5 evidence](AC8/Evidence/AC8.5/verification.md), and [AC8.6 evidence](AC8/Evidence/AC8.6/verification.md). Standalone dismissal is a separately verified user-requested extension governed by D1 in the [focused plan](../superpowers/plans/2026-09-20-ac8-6-whole-roster-eligibility.md); it does not expand the base AC8.6 definition.
...
- [x] AC8.4 — Before habitats are implemented, every existing town counts as Goblin habitat and offers only Goblin classes absent from the current roster. This works on the existing map and after reload, without requiring habitat generation or changing town count, placement, roads or spawns. Habitat-enabled worlds later use explicit clan ownership. ([current-world verification](AC8/Evidence/AC8.6/verification.md))
- [x] AC8.4a - Ownership prerequisite: every existing v1 town resolves to Goblin ownership before and after reload; invalid coordinates, non-town queries and unsupported versions fail explicitly without changing topology. ([verification](AC8/Evidence/AC8.4/verification.md))
- [x] AC8.4b - Habitat and world diagnostics expansion: the HUD shows the committed current hex's habitat, and a collapsed battle-style world drawer shows coordinates, terrain, base/effective encounter, habitat/ownership, map context and run/persistence state. Diagnostics are read-only, isolate pointer/keyboard input, close for modal surfaces and reset across sessions. V1 is explicitly labelled `Legacy world v1 rule`; generated habitat IDs remain absent until AC9. ([verification and screenshots](AC8/Evidence/AC8.4-expansion/verification.md))
- [x] AC8.5 — Recruitment is available in intact allied towns at exactly 500g per character. Each town offers only recruitable classes from its habitat's clan. Verified for the explicit legacy v1 Goblin-town policy; generated ownership and burned/hostile state integration remain AC9/AC10. ([verification](AC8/Evidence/AC8.5/verification.md))
- [x] AC8.6 — Towns offer only classes absent from the whole current roster, including passed-out members. Offers refresh after recruitment, dismissal/replacement and reload; a class becomes eligible again when its final roster member leaves. Empty eligibility shows no purchasable offer, and stale requests cannot bypass this rule. ([verification](AC8/Evidence/AC8.6/verification.md))
...
| `AC8.4 / AC8.4a` | Current-world ownership and filtered offers PASS | [Ownership evidence](AC8/Evidence/AC8.4/verification.md) plus [AC8.6 evidence](AC8/Evidence/AC8.6/verification.md) verify all seven v1 towns, exact filtered offers before/after Continue and unchanged topology. Generated explicit ownership remains AC9. |
| `AC8.4b` | Habitat/HUD/drawer implementation and verification PASS | Implementation `cf25a14`: 27 regression runners, actual launcher movement/Continue, pointer/keyboard isolation, and inspected 1280x720 / 1920x1080 captures. See [expansion evidence](AC8/Evidence/AC8.4-expansion/verification.md). No generated habitats or recruitment acceptance is implied. |
| `AC8.2-AC8.3 revisit regression` | Completed-combat replay fix PASS | Implementation `51b8642`: `Tests/WorldMap/test_cleared_battle_hex_revisit.gd` reproduces victory, acknowledgement, leave/return, stale requests and reload. Seventeen targeted executions plus live QA verify safe revisits, unchanged gold and preserved unfinished combat. See [regression evidence](AC8/Evidence/battle-revisit-fix/verification.md). |
| `AC8.5` | Current-world recruitment PASS | Implementation `7e51140`: 21 automated suites, six separate-process restart checks, and rendered production-launcher input at 1280x720/1920x1080. Verifies shared availability, exact 500g price, clan allowlist, open/reopen and close/cancel no-op behavior. Supporting replacement/retry checks do not confer AC8.6-AC8.8 acceptance. See [evidence](AC8/Evidence/AC8.5/verification.md). |
| `AC8.6` | Whole-roster eligibility PASS | [Evidence](AC8/Evidence/AC8.6/verification.md): 23 automated suites, eight restart processes and two rendered production-launcher runs; canonical class exclusion, passed-out/recovered members, refresh, final representative, empty offers and stale requests. User-requested standalone dismissal and D1 policy are verified separately. |
| `AC8.7-AC8.8` | Full capacity and transaction acceptance pending | Complete capacity, cancellation, failed-save retry, duplicate settlement and actual ruins rejection matrices. Supporting AC8.5/AC8.6 checks do not confer these acceptance criteria. |
