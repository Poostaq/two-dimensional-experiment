...
- [ ] AC3.5 — After battle victory, characters who passed out return for the next battle at 50% health, with lower recovery allowed on harder difficulties
...
| `AC3.5` | Manual runtime check | Let a character pass out during battle, win the battle, and verify that character begins the next battle at 50% HP or the configured lower value on harder difficulties. |
