...
**Acceptance Criterion:** AC1.4 — Entering the boss hex automatically triggers final battle  
**Owner:** Project Lead  
**Prepared by:** Codex  
**Date:** 2026-07-24  
...
Open an `Encounter` overlay immediately after every accepted move onto a map hex. The overlay provides a reusable transition boundary for future Safe, Combat, and Boss encounter implementations while satisfying AC1.4 through automatic Boss-hex entry.
...
The controller prevents additional movement while an encounter overlay is active. When the overlay requests closure, the controller removes it, clears the active-encounter reference, and permits navigation again.
...
AC1.4 completion evidence is stored under `Docs/Specs/AC1/Evidence/AC1.4/YYYY-MM-DD/` and includes the current test log, manual runtime observations, and implementation link required by repository governance.
...
- Boss entry requires no separate confirmation and is covered by current automated and manual evidence.
